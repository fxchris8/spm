...
KELOMPOK = {
    'container' : ["KM. ORIENTAL EMERALD", "KM. ORIENTAL RUBY", "KM. ORIENTAL SILVER", "KM. ORIENTAL GOLD", "KM. ORIENTAL JADE", "KM. ARMADA SEJATI", 
                        "KM. ORIENTAL DIAMOND", "KM. LUZON", "KM. BALI AYU", "KM. VERIZON", "KM. ORIENTAL GALAXY", "KM. HIJAU SAMUDRA", "KM. ARMADA PERMATA",
                        "KM. ORIENTAL SAMUDERA", "KM. ORIENTAL PACIFIC", "KM. PULAU NUNUKAN", "KM. TELUK FLAMINGGO", "KM. TELUK BERAU",
                        "KM. TELUK BINTUNI", "KM. PULAU LAYANG", "KM. PULAU WETAR", "KM. PULAU HOKI", "KM. SPIL HANA", "KM. SPIL HASYA",
                        "KM. SPIL HAPSRI", "KM. SPIL HAYU", "KM. HIJAU JELITA", "KM. HIJAU SEJUK", "KM. ARMADA SEJATI", "KM. ARMADA SERASI", "KM. ARMADA SEGARA",
                        "KM. ARMADA SENADA", "KM. HIJAU SEGAR", "KM. TITANIUM", "KM. VERTIKAL", "KM. SPIL RENATA", "KM. SPIL RATNA", "KM. SPIL RUMI", "KM. PEKAN BERAU",
                        "KM. SPIL RAHAYU", "KM. SPIL RETNO", "KM. MINAS BARU", "KM. PEKAN SAMPIT", "KM. SELILI BARU", "KM. DERAJAT", "KM. MULIANIM", "KM. PRATIWI RAYA",
                        "KM. MAGELLAN", "KM. PAHALA", "KM. PEKAN RIAU", "KM. PEKAN FAJAR", "KM. PEKAN BERAU", "KM. FORTUNE", "KM. PRATIWI SATU", "KM. BALI AYU", 
                        "KM. BALI GIANYAR", "KM. BALI KUTA", "KM. BALI SANUR", "KM. AKASHIA", "KM. KAPPA"],
    'manalagi'   : ["KM. MANALAGI ASTA", "KM. MANALAGI ASTI", "KM. MANALAGI DASA", "KM. MANALAGI ENZI", "KM. MANALAGI HITA", "KM. MANALAGI SAMBA", "KM. MANALAGI TARA",
                         "KM. MANALAGI TISYA", "KM. MANALAGI VIRA", "KM. MANALAGI WANDA", "KM. MANALAGI YASA", "KM. XYS SATU"],
    'bc' : ["BC. ANGSA LAUT", "BC. BALIKPAPAN RAYA", "BC. BANJARMASIN RAYA", "BC. BAYA", "BC. BELAWAN RAYA", "BC. EPSILON", "BC. GAJAH LAUT", "BC. GAJAH MADA",
            "BC. KAIMANA INDAH", "BC. MURO 5", "BC. SAMARINDA RAYA", "BC. SHORYU BARU", "BC. SURABAYA RAYA", "BC. TARAKAN RAYA", "BC. TENYO MARU"],
    'mt' : ["MT. GLOBAL", "MT. PANTAI LAMONG"],
    'tb' : ["TB. ALPHA", "TB. CAPUNG I", "TB. CAPUNG II", "TB. CAPUNG III", "TB. GAMMA SATU", "TB. MANGGA RAYA", "TB. SPIL BOAT", "TB. TOYO", "TB. YITNA YUWANA", "TB. YUSHIN MARU"],
...
def calculate_day_remains_difference(df):
    ...
    df['day_remains'] = pd.to_datetime(df['day_remains'], format='%Y-%m-%d', errors='coerce')
    ...
    df['day_remains'] = (df['day_remains'].dt.date - today).apply(lambda x: x.days if pd.notna(x) else None)
...
def getRecommendation(df, dataCandidates, bagian, vessel_name, rank, certificate, age_range):
    ...
    filtered_df['combined_features'] = filtered_df['last_position'] + " " + filtered_df['certificate']
    ...
    def get_word2vec_vector(text):
        ...
        if word_vectors:
            ...
            return np.zeros(word2vec_model.vector_size)  # Vektor nol jika kata tidak ditemukan
    ...
    def get_hierarchy_score(cert1, cert2):
        for group in hierarchy_mapping.values():
            if cert1 in group and cert2 in group:
                ...
                return 1 - abs(idx1 - idx2) / len(group)  # Skor berdasarkan jarak hierarki
    ...
    filtered_df['rank_match'] = filtered_df['last_position'].apply(lambda r: 1 if r == rank else 0)
    ...
    filtered_df['non_hierarchical_score'] = filtered_df['certificate'].apply(handle_non_hierarchical_certificate)
    ...
    filtered_df = filtered_df.sort_values(by=['rank_match', 'final_score'], ascending=[False, False])

    # Mengambil top 20 rekomendasi
    recommendations = filtered_df.head(20)[[
        'seamancode', 'seafarercode', 'name', 'last_position', 'last_location', 
        'VESSEL GROUP ID', 'age', 'certificate', 'similarity_score', 'phone_number_1', 'phone_number_2', 'phone_number_3', 'phone_number_4', 'day_remains'
    ]]
...
def vessel_group_id_deck(dataframe, vessel, type=None):
    ...
    CONTAINER_DECK = {
        'container_rotation1': ["KM. ORIENTAL EMERALD", "KM. ORIENTAL RUBY", "KM. ORIENTAL SILVER", 
                                "KM. ORIENTAL GOLD", "KM. ORIENTAL JADE", "KM. ORIENTAL DIAMOND"],
        'container_rotation2': ["KM. LUZON", "KM. VERIZON", "KM. ORIENTAL GALAXY", "KM. HIJAU SAMUDRA", 
                                "KM. ARMADA PERMATA"],
        'container_rotation3': ["KM. ORIENTAL SAMUDERA", "KM. ORIENTAL PACIFIC", "KM. PULAU NUNUKAN", 
                                "KM. TELUK FLAMINGGO", "KM. TELUK BERAU", "KM. TELUK BINTUNI"],
        'container_rotation4': ["KM. PULAU LAYANG", "KM. PULAU WETAR", "KM. PULAU HOKI", "KM. SPIL HANA", 
                                "KM. SPIL HASYA", "KM. SPIL HAPSRI", "KM. SPIL HAYU"],
        'container_rotation5': ["KM. HIJAU JELITA", "KM. HIJAU SEJUK", "KM. ARMADA SEJATI", "KM. ARMADA SERASI", 
                                "KM. ARMADA SEGARA", "KM. ARMADA SENADA", "KM. HIJAU SEGAR", "KM. TITANIUM", 
                                "KM. VERTIKAL"],
        'container_rotation6': ["KM. SPIL RENATA", "KM. SPIL RATNA", "KM. SPIL RUMI", "KM. PEKAN BERAU", 
                                "KM SPIL RAHAYU", "KM. SPIL RETNO", "KM. MINAS BARU", "KM PEKAN SAMPIT", 
                                "KM. SELILI BARU"],
        'container_rotation7': ["KM. DERAJAT", "KM. MULIANIM", "KM. PRATIWI RAYA", "KM. MAGELLAN", 
                                "KM. PAHALA", "KM. PEKAN RIAU", "KM. PEKAN FAJAR", "KM. FORTUNE"],
        'container_rotation8': ["KM. PRATIWI SATU", "KM. BALI SANUR", "KM. BALI KUTA", "KM. BALI GIANYAR", 
                                "KM. BALI AYU", "KM. AKASHIA", "KM KAPPA"]
    }
    ...
    for idx, (group_name, vessels) in enumerate(groups.items(), start=1):
        group_id = f"{prefix}{idx}"  # Format group ID seperti D1, D2, ... atau E1, E2, ...
        for vessel in vessels:
            vessel_to_group[vessel] = group_id
    
    # Tambahkan kolom baru berdasarkan mapping
    dataframe['VESSEL GROUP ID'] = dataframe['last_location'].map(vessel_to_group).fillna('UNKNOWN')
    ...
    cols.insert(vessel_idx + 1, cols.pop(cols.index('VESSEL GROUP ID')))  # Pindahkan 'VESSEL GROUP ID' setelah 'VESSEL'
