...
KELOMPOK = {
      ...
      'others' : ["DARAT", "DARAT BIASA", "DARAT STAND-BY", "Stand by Crew", "PENDING CUTI", "PENDING GAJI", "PENDING CUTI"]
  }
...
def get_schedule(vessel_group_id_filter, new_nahkoda, type, part):
  ...
  filtered_df_nahkoda = filtered_df[(filtered_df['last_position'] == 'NAKHODA') & (filtered_df['VESSEL GROUP ID'] == vessel_group_id_filter)]
  ...
  filtered_df_nahkoda['end_date'] = pd.to_datetime(filtered_df_nahkoda['end_date'], errors='coerce', dayfirst=True)
  ...
  bulan_list = [f"{month} {year}" for year in range(min_start_year, min_start_year + 3)
  ...
  if new_nahkoda:
      ...
      filtered_df_nahkoda = pd.concat([cadangan, filtered_df_nahkoda], ignore_index=True, sort=False)
  ...
  while month_index < len(bulan_list):
      ...
      for i, kapal in enumerate(kapal_list):
          if pd.isna(schedule.at[kapal, month]) and not transaction:  # **Pastikan hanya ada 1 transaksi per bulan**
              if available_nahkoda:  # Jika masih ada nakhoda yang tersedia
                  nakhoda = available_nahkoda.pop(0)  # Ambil nakhoda pertama dari daftar
...
def get_nahkoda(vessel_group_id_filter, new_nahkoda, type, part, quantity="ALL"):
    ...
    if quantity != "ONE":
        filtered_df = filter_in_vessel(local_df, type, KELOMPOK)  # Ganti KELOMPOK sesuai konteksmu
        filtered_df = vessel_group_id_deck(filtered_df, type, part)
    ...
    if 'end_date' in filtered_df_nahkoda.columns:
        ...
        filtered_df_nahkoda = filtered_df_nahkoda.sort_values(by='end_date', ascending=True)
        filtered_df_nahkoda['end_date'] = filtered_df_nahkoda['end_date'].dt.strftime('%d/%m/%Y')
    else:
    ...
    if cadangan_list:
        ...
        filtered_df_nahkoda = pd.concat([filtered_df_nahkoda, cadangan_df], ignore_index=True, sort=False)
    ...
    return filtered_df_nahkoda[['Index', 'name', 'last_location', 'seamancode', 'start_date', 'end_date']]
