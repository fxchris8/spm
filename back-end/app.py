from flask import Flask, session, render_template, request,json, jsonify, send_file, redirect, url_for
from apscheduler.schedulers.background import BackgroundScheduler
from request_api import fetch_and_save_data  # Import the function from request_api.py
from flask_cors import CORS
import pandas as pd
from io import BytesIO
from model import getRecommendation, search_candidate, filter_in_vessel, vessel_group_id_deck
...
import os
from request_api import get_schedule, fetch_and_save_data, get_nganggur, get_nahkoda, fetch_and_save_mutasi_data
...
def schedule_fetch_task():
    ...
    scheduler.add_job(fetch_and_save_mutasi_data, 'interval', hours=24)  # Runs every 24 hours
    scheduler.start()
...
df_filtered = combined_df[combined_df['DAY REMAINS DIFF'] > 0][['seamancode', 'seafarercode', 'name', 'last_position', 'last_location', 'age', 'certificate', 'DAY REMAINS DIFF']]
...
def get_top_5_similar(target_seaman_code):
    try:
        ...
        target_seaman_data = combined_df[combined_df['seamancode'] == target_seaman_code]
        
        if target_seaman_data.empty:
            print("No seaman found with that code")
            return jsonify({"error": f"Seaman dengan kode {target_seaman_code} tidak ditemukan"})
        ...
        filtered_candidates = combined_df[combined_df['seamancode'] != target_seaman_code].copy()
        
        filtered_candidates['vector'] = (filtered_candidates['last_position'].astype(str) + " " + 
                                      filtered_candidates['certificate'].astype(str))
        filtered_candidates['vector'] = filtered_candidates['vector'].apply(get_word2vec_vector)
        
        filtered_candidates['vector'] = filtered_candidates['vector'].apply(lambda x: x.tolist())

        filtered_candidates['similarity'] = filtered_candidates['vector'].apply(
            lambda x: float(cosine_similarity([user_vector], [x])[0][0])
        )
        
        filtered_candidates = filtered_candidates.sort_values(by='similarity', ascending=False)
        top_5_recommendations = filtered_candidates.head(5)
        
        columns_to_drop = ['vector', 'phone_number_1', 'phone_number_2', 
                          'phone_number_3', 'phone_number_4', 'experience']
...
def get_dashboard_data():
    ...
    data = data[['SEAMAN CODE', 'SEAFARER CODE', 'SEAMAN NAME', 'RANK', 'VESSEL', 'UMUR', 'CERTIFICATE', 'DAY REMAINS']]
...
def get_mutasi_filtered():
    try:
        ...
        lokasi_filter = ["PENDING CUTI", "PENDING GAJI", "DARAT BIASA", "DARAT", "DARAT STAND-BY"]
        ...
        df_mutasi_filtered = df_history[df_history["seamancode"].isin(seamancode_terfilter)]
        ...
        mutasi_dict_filtered = (
            ...
            .apply(lambda g: {
                "name": g["name"].iloc[0],
                "vessels": g.loc[~g["fromvesselname"].isin(lokasi_filter), "fromvesselname"].dropna().unique().tolist()
            })
            .to_dict()
        )
...
def get_options():
    ...
    rank_order = [
        "NAKHODA", "MUALIM I", "MUALIM II", "MUALIM III", "KKM", 
        "MASINIS I", "MASINIS II", "MASINIS III", "SERANG", 
        "JURU MUDI", "ELECTRICIAN", "MANDOR MESIN", "JURU MINYAK", 
        "JURU MASAK I", "KADET DEK", "KADET MESIN", "EXTRA KKM", 
        "KADET ELECTRONIC", "EXT. MUALIM I"
    ]
    
    cert_order = [
        "ANT-I", "ANT-II", "ANT-III", "ANT-IV", "ANT-V", "ANT-D", "ATT-I", "ATT-II", 
        "ATT-III", "ATT-IV", "ATT-V", "ATT-D", "BASIC SAFETY TRAINING", "ETO"
    ]
    
     # Pilihan untuk setiap kolom
    bagian_option = copy_df["BAGIAN"].unique().tolist() if "BAGIAN" in copy_df.columns else []
    cert_option = copy_df["certificate"].unique().tolist() if "certificate" in copy_df.columns else []
    rank_option = copy_df["last_position"].unique().tolist() if "last_position" in copy_df.columns else []
    vessel_option = copy_df["last_location"].unique().tolist() if "last_location" in copy_df.columns else []
    
    # Urutkan rank_option berdasarkan rank_order
    rank_option = sorted(rank_option, key=lambda x: rank_order.index(x) if x in rank_order else len(rank_order))
    
    # Urutkan cert_option berdasarkan cert_order
    cert_option = sorted(cert_option, key=lambda x: cert_order.index(x) if x in cert_order else len(cert_order))
...
def get_recommendation():
    ...
    recommendations = getRecommendation(original_df, data_candidate, bagian, vessel_name, rank, certificate, age_range)
...
def get_manual_search():
    ...
    recommendations = getRecommendation(copy_df, data_candidate, bagian, vessel_name, data_candidate["RANK"], data_candidate["CERTIFICATE"], age_range)
    
    # Ensure PHONE1, PHONE2, PHONE3, and PHONE4 are included in the response
    result = recommendations[['seamancode', 'seafarercode', 'name', 'last_position', 'last_location', 'age', 'certificate', 'phone_number_1', 'phone_number_2', 'phone_number_3', 'phone_number_4', 'day_remains']].to_dict(orient="records")
    ...
    try:
        ...
        result = (
            ...
            .apply(lambda g: {
                ...
                ]["fromvesselname"].dropna().unique().tolist()
            })
            .tolist()
        )
...
def get_promotion_candidates_kkm():
    try:
        ...
        kapal_disyaratkan = {
            "KM. HIJAU SEJUK", "KM. ORIENTAL DIAMOND", "KM. ORIENTAL RUBY", "KM. ORIENTAL JADE",
            "KM. VERIZON", "KM. SPIL HANA", "KM. SPIL HAPSRI", "KM. SPIL HAYU", "KM. SPIL HASYA",
            "KM. HIJAU JELITA", "KM. HIJAU SAMUDERA", "KM. ORIENTAL GOLD", "KM. ORIENTAL GALAXY",
            "KM. LUZON", "KM. ARMADA PERMATA", "KM. ORIENTAL SILVER", "KM. ORIENTAL EMERALD"
        }
...
def save_excel():
    ...
    try:
        ...
        base_dir = pathlib.Path(__file__).parent.parent.resolve()  # ke dua tingkat atas dari app.py
        ...
        return jsonify({"status": "success", "message": f"File berhasil disimpan!"})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
...
def filter_history():
    ...
    allowed_status = ["PENDING CUTI", "PENDING GAJI", "DARAT BIASA", "DARAT", "DARAT STAND-BY"]
    ...
    try:
        ...
        if "history" not in df.columns or "code" not in df.columns:
            return jsonify({"status": "error", "message": "Kolom 'history' atau 'code' tidak ditemukan."}), 400
