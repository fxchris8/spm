...
def migrate():
    ...
    try:
        ...
        if 'transactiondate' in df_mutations.columns:
            df_mutations['transactiondate'] = df_mutations['transactiondate'].apply(parse_date)
        ...
        with engine.connect() as conn:
            # Primary keys dulu
            try:
                conn.execute(text("ALTER TABLE seamen ADD CONSTRAINT pk_seamen PRIMARY KEY (seamancode)"))
            ...
            try:
                conn.execute(text("ALTER TABLE mutations ADD CONSTRAINT pk_mutations PRIMARY KEY (mutationnoid)"))
            ...
            if orphaned_records:
                ...
                orphaned_df = pd.DataFrame(orphaned_records, columns=['seamancode', 'seamanname', 'mutation_count'])
                ...
                with open(txt_path, 'w', encoding='utf-8') as f:
                    ...
                    f.write(f"Migration Script: migrate.py\n\n")
                    f.write("-" * 80 + "\n\n")
                    f.write("SUMMARY\n")
                    f.write("-" * 80 + "\n")
                    f.write(f"Total seamen with orphaned records: {len(orphaned_records)}\n")
                    f.write(f"Total orphaned mutation records: {sum(r[2] for r in orphaned_records)}\n\n")
                    f.write("-" * 80 + "\n\n")
                    ...
                    f.write("NOTES\n")
                    f.write("-" * 80 + "\n")
                    f.write("These seamen have mutation history in the database but do not exist\n")
                    ...
                    f.write("-" * 80 + "\n")
                    f.write("All orphaned mutation records have been deleted to maintain\n")
                    f.write("referential integrity and enable foreign key constraints.\n\n")
                ...
                print(f"Found {len(orphaned_records)} seamen with {total_mutations} orphaned mutations")
        ...
        with engine.connect() as conn:
            ...
            mutations_count = conn.execute(text("SELECT COUNT(*) FROM mutations")).scalar()
