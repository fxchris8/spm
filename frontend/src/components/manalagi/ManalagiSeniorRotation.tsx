...
export function ManalagiSeniorRotation({
  ...
  const getGroupDisplayName = (groupKey: string | null): string => {
    if (!groupKey) return '';
    const groupNumber = groupKey.replace('manalagi_rotation', '');
    return `Group ${groupNumber} Manalagi ${getJobDisplayName(job)}`;
  };
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-1">
            <h1 className="text-3xl font-bold text-gray-800">
              Generate Manalagi Schedule - {getJobDisplayName(job)}
            </h1>
                ...
                <div className="flex flex-wrap gap-2">
                  {affectedGroups.map((groupKey: string) => (
                    <Badge key={groupKey} color="warning">
                      {groupKey.replace('manalagi_rotation', 'Grup ')}
                    </Badge>
                  ))}
                </div>
              </div>
                ...
                <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  {(() => {
                    ...
                    const showForKKMorMasinisII =
                      isKKMorMasinisII &&
                      (selectedGroup === 'manalagi_rotation1' ||
                        selectedGroup === 'manalagi_rotation2');
