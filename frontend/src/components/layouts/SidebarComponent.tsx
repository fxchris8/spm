...
// Theme for sub-level collapse (Container, Manalagi, BC) - Level 2 (15px) text-gray-900 with enlarged w-5.5 icons
const subCollapseTheme: ComponentProps<typeof Sidebar.Collapse>['theme'] = {
  ...
  icon: {
    base: 'h-5.5 w-5.5 flex-shrink-0 text-gray-900 transition duration-75 group-hover:text-gray-900 dark:text-gray-100',
    open: {
      off: 'text-gray-900',
      on: 'text-gray-900',
    },
...
// Theme for leaf items (Senior, Junior) - Level 3 (14px) text-gray-900 with w-5.5 icons
const subItemTheme: ComponentProps<typeof Sidebar.Item>['theme'] = {
  ...
  icon: {
    base: 'h-5.5 w-5.5 flex-shrink-0 text-gray-900 group-hover:text-gray-900 dark:text-gray-200',
...
function getInitialOpenState(pathname: string): SidebarOpenState {
  const isContainerActive = pathname.startsWith('/senior-rotation') || pathname.startsWith('/junior-rotation');
  const isManalagiActive = pathname.startsWith('/manalagi-senior-rotation') || pathname.startsWith('/manalagi-junior-rotation');
  const isBcActive = pathname.startsWith('/bc-senior-rotation') || pathname.startsWith('/bc-junior-rotation');
  ...
  try {
    ...
    if (saved) {
      ...
      return {
        ...
        rotation_container: parsed.rotation_container ?? isContainerActive,
        rotation_manalagi: parsed.rotation_manalagi ?? isManalagiActive,
        rotation_bc: parsed.rotation_bc ?? isBcActive,
  ...
  return {
    ...
    rotation: true,
    rotation_container: isContainerActive,
    rotation_manalagi: isManalagiActive,
    rotation_bc: isBcActive,
    ...
    <Sidebar aria-label="Sidebar" className="w-full">
      <Sidebar.Items>
        <Sidebar.ItemGroup>
          ...
          >
            <Sidebar.Collapse
              icon={() => <HiOutlineCube className="w-5.5 h-5.5 flex-shrink-0" />}
            ...
            >
              <Sidebar.Item
                ...
                icon={() => <HiOutlineUserGroup className="w-5.5 h-5.5 flex-shrink-0" />}
              ...
              <Sidebar.Item
                ...
                icon={() => <HiOutlineUser className="w-5.5 h-5.5 flex-shrink-0" />}
            ...
            <Sidebar.Collapse
              icon={() => <GiShipWheel className="w-5.5 h-5.5 flex-shrink-0" />}
              label="Manalagi"
            ...
            <Sidebar.Collapse
              icon={() => <GiCargoCrane className="w-5.5 h-5.5 flex-shrink-0" />}
