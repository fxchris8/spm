...
export function useMutasiData(
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['mutasi-data', job, type, groupKey, lockedCadanganCodes, forecastMonth],
    queryFn: async () => {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const getPromotionEndpoint = (job: string): string => {
        ...
        return base[job] || `${API_BASE_URL}/seamen/promotion-candidates-nakhoda`;
      };
...
export function useCadanganData(
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['cadangan-data', job, groupKey, lockedCadanganCodes, forecastMonth, categorization],
...
export function usePromotionCandidates(
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['promotion-candidates', job, groupKey, lockedCadanganCodes, forecastMonth, categorization],
    queryFn: () =>
